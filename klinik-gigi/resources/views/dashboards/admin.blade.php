    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3">
            <div class="card-dashboard p-3 d-flex align-items-center gap-3 border-start border-primary border-4">
                ...
                <div><small class="text-muted d-block">Total Pasien</small>
                    <h5 class="fw-bold mb-0">{{ $totalPasien }}</h5>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card-dashboard p-3 d-flex align-items-center gap-3 border-start border-success border-4">
                ...
                <div><small class="text-muted d-block">Booking Hari Ini</small>
                    <h5 class="fw-bold mb-0">{{ $totalBookingToday }}</h5>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card-dashboard p-3 d-flex align-items-center gap-3 border-start border-warning border-4">
                ...
                <div><small class="text-muted d-block">Omzet (Bulan)</small>
                    <h5 class="fw-bold mb-0 text-truncate" style="max-width: 130px;">Rp
                        {{ number_format($totalPendapatan, 0, ',', '.') }}</h5>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card-dashboard p-3 d-flex align-items-center gap-3 border-start border-info border-4">
                ...
                <div><small class="text-muted d-block">Tim Medis</small>
                    <h5 class="fw-bold mb-0">{{ $totalDokter }}</h5>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-8">
            <div class="card-dashboard p-4 h-100">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h6 class="fw-bold m-0"><i class="fa-solid fa-list-check me-2 text-primary"></i>Booking Antrian Terbaru
                    </h6>
                    <a href="{{ route('admin.booking') }}" class="btn btn-sm btn-link text-decoration-none">Lihat Semua <i
                            class="fa-solid fa-arrow-right small"></i></a>
                </div>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <small class="text-muted"
                                            style="font-size: 0.75rem;">{{ $booking->pasien->NoTelp ?? '-' }}</small>
                                    </td>
                                    <td>
                                        <div class="small fw-semibold text-dark">{{ $booking->jadwal->dokter->Nama ?? '-' }}
                                        </div>
                                        <div class="text-muted" style="font-size: 0.7rem;">{{ $booking->jadwal->Tanggal }} |
                                            {{ $booking->jadwal->sesi ?? '-' }}</div>
                                    </td>
                                    <td>
                                        @php
                                        ...
                                        @endphp
                                        <span
                                            class="badge bg-{{ $badgeClass }}-subtle text-{{ $badgeClass }} border-0 px-2 rounded-pill"
                                    ...
                                    <td class="text-end">
                                        @if($booking->Status == 'PRESENT')
                                            <button type="button" class="btn btn-sm btn-outline-danger py-1 px-3 rounded-pill"
                                                ...
                                                onclick="confirmCancel('{{ route('admin.booking.destroy', $booking->IdBooking) }}', '{{ $booking->pasien->Nama }}')">Cancel</button>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-5 text-muted small">Belum ada antrian aktif saat ini.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                @forelse($topTindakan as $index => $t)
                    <div class="mb-4">
                        ...
                        <div class="progress shadow-none stat-bar bg-light">
                            <div class="progress-bar bg-{{ $colors[$index] ?? 'primary' }}"
                                style="width: {{ ($t->count / max(1, $totalPemeriksaan)) * 100 }}%"></div>
                        </div>
                    </div>
                @empty
                    ...
                    @forelse($auditActivities as $audit)
                        <div class="audit-item {{ $loop->first ? 'active' : '' }}">
                            <div class="d-flex align-items-center gap-2 mb-1">
                                <i class="fa-solid {{ $audit['icon'] }} text-{{ $audit['color'] }} small"
                            ...
                            <small class="text-primary-emphasis opacity-75"
                                style="font-size: 0.65rem;">{{ $audit['time'] }}</small>
                        </div>
                    @empty
