...
@section('sidebar-menu')
    <a href="{{ route('dokter.dashboard') }}" class="nav-link active"><i class="fa-solid fa-stethoscope"></i> Praktek Hari
        Ini</a>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    <div class="card-custom bg-info-subtle border-0 h-100">
                        ...
                        @if($jadwalHariIni->isNotEmpty())
                            <h5 class="fw-bold mb-0 text-info-emphasis">{{ \Carbon\Carbon::today()->isoFormat('dddd, D MMM') }}
                            </h5>
                ...
                <div class="col-md-3">
                    <div
                        class="card-custom bg-warning-subtle border-0 h-100 position-relative border-start border-warning border-4">
            ...
            <div class="card-custom mb-4">
                ...
                <div class="list-group list-group-flush rounded-3">
                    @forelse($antrian as $index => $booking)
                        ...
                        <div
                            class="list-group-item p-3 mb-2 rounded border {{ $index == 0 && $booking->Status == 'PRESENT' ? 'border-primary bg-primary-subtle' : 'bg-white' }}">
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="d-flex align-items-center gap-3">
                                    <div class="{{ $index == 0 ? 'bg-primary text-white' : 'bg-light text-secondary' }} fs-5 fw-bold rounded-circle d-flex align-items-center justify-content-center"
                                        style="width: 45px; height: 45px;">
                                        {{ $index + 1 }}
                                    </div>
                                    <div>
                                        <h6 class="fw-bold mb-1">{{ $booking->pasien->Nama ?? 'Nama Pasien' }} <span
                                                class="badge bg-secondary ms-2 small"
                                                style="font-size: 0.6em;">{{ $booking->IdBooking }}</span></h6>
                                ...
                                @if($booking->Status == 'PRESENT')
                                    <a href="{{ route('dokter.rekam-medis.create', $booking->IdBooking) }}"
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                ...
                                <button type="button" class="btn btn-sm btn-link p-0 text-decoration-none"
                                    style="font-size: 0.8rem;" onclick="copyToClipboard(this)">
                                    <i class="fa-regular fa-copy me-1"></i>Salin
                                </button>
                            </div>
                            <textarea id="quickDiagnosa" class="form-control" rows="3"
                                placeholder="Tulis diagnosa sementara..."></textarea>
                        </div>
