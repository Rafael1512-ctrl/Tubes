...
@section('header-subtitle', 'Informasi terbaru mengenai aktivitas praktek Anda.')
...
@section('sidebar-menu')
    ...
    <a href="{{ route('dokter.notifications') }}" class="nav-link active"><i class="fa-solid fa-bell"></i> Notifikasi</a>
@endsection
            ...
            <div class="card-custom bg-white border-0 shadow-sm overflow-hidden p-0">
                <div class="list-group list-group-flush">
                    @forelse($notifications as $n)
                        <div
                            class="list-group-item p-4 border-light-subtle transition-all @if(!$n->read_at) bg-primary bg-opacity-10 @endif">
                            <div class="d-flex justify-content-between align-items-start">
                                <div class="d-flex gap-4">
                                    <div class="bg-white p-3 rounded-4 shadow-sm text-primary">
                                        <i
                                            class="fa-solid fa-{{ ($n->data['type'] ?? '') == 'broadcast' ? 'bullhorn' : 'bell' }} fa-lg"></i>
                                    </div>
                                    <div>
                                ...
                                <div class="d-flex flex-column align-items-end gap-2">
                                    @if(!$n->read_at)
                                        ...
                                        <form action="{{ route('dokter.notifications.read', $n->id) }}" method="POST">
                                            @csrf
                                            <button type="submit"
                                                class="btn btn-sm btn-link text-primary p-0 fw-bold text-decoration-none">
                                                Tandai telah dibaca
                                            </button>
                                        </form>
                                    @else
                                        <span class="text-muted small italic"><i class="fa-solid fa-check-double me-1"></i>
                                            Dibaca</span>
                                    @endif
                                </div>
                            </div>
